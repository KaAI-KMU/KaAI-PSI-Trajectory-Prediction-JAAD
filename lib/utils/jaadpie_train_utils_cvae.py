...
def train(model, train_gen, criterion, optimizer, device):
    ...
    with torch.set_grad_enabled(True):
        for batch_idx, data in enumerate(loader):
            ...
            all_goal_traj, cvae_dec_traj, KLD_loss, _  = model(data, inputs=input_traj, map_mask=None, targets=target_traj)
...
def val(model, val_gen, criterion, device):
    ...
    with torch.set_grad_enabled(False):
        for batch_idx, data in enumerate(loader):
            ...
            all_goal_traj, cvae_dec_traj, KLD_loss, _ = model(data, inputs=input_traj, map_mask=None, targets=None,training=False)
            cvae_loss = cvae_multi(cvae_dec_traj,target_traj)
